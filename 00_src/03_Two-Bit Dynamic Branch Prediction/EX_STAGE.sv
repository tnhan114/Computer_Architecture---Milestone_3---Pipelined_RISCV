...
module branch_unit(
    ...
    input logic pred_taken_EX,       // BTB dự đoán có nhảy không?
    input logic [31:0] pc_pred_EX,   // BTB dự đoán nhảy về đâu?
    input logic [31:0] alu_result_EX,// Thực tế nhảy về đâu?
    output logic mispredict, taken_EX
);
    logic branch_cond_met; // dieu kien re nhanh (beq, bne...) thoa man
    logic actual_taken;    // thuc te có nhay khong

    // xác định xem dieu kien branch (B-type) co thoa man khong
    always_comb begin
        branch_cond_met = 1'b0;
        if (is_branch_EX) begin
            case (type_sel_EX)
                3'b000: branch_cond_met = br_equal_EX;  // beq
                3'b001: branch_cond_met = ~br_equal_EX; // bne
                3'b100: branch_cond_met = br_less_EX;   // blt
                3'b101: branch_cond_met = ~br_less_EX;  // bge
                3'b110: branch_cond_met = br_less_EX;   // bltu
                3'b111: branch_cond_met = ~br_less_EX;  // bgeu
                default: branch_cond_met = 1'b0;
            endcase
        end
    end

    // xac dinh thuc te co nhay hay khong
    // nhay khi: la lenh branch va thoa dieu kien hoac la jal hoac la jalr
    assign actual_taken = (is_branch_EX & branch_cond_met) | is_jal_EX | is_jalr_EX;

    // logic Output cho BTB update
    assign taken_EX = actual_taken;

    // du doan sai khi:
    //  - lech pha: thuc te phai nhay ma btb doan khong nha, hoac la thuc te khong nhay ma btb doan nhay.
    //  - sai dia chi nhay: thuc te va du doan deu nhay nhung dia chi nhay du doan khong giong dia chi nhay chinh xac
    assign mispredict = (actual_taken ^ pred_taken_EX) |  
                        (actual_taken & pred_taken_EX & (alu_result_EX != pc_pred_EX));
...
module EX_MEM(
	...
	input logic pc_taken_EX,//=mispredict de tao tin hieu cho o_mispred
	///
