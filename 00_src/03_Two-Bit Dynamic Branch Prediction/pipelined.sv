...
module pipelined(
    ...
    logic mispredict;           // 1: Nhảy/Rẽ nhánh thành công (Taken)
    ...
    logic [1:0] wb_sel_wb;
	  ...
	  // ========================================================================
    // 2. KHỐI PHÁT HIỆN HAZARD (HAZARD DETECTION UNIT)
    // ========================================================================
    hazard_unit HAZARD_UNIT(
	 ...
	 // ========================================================================
    // 3. KHỐI FORWARDING (FORWARDING UNIT)
    // ========================================================================
    ...
    // 4. TẦNG IF 
    // ========================================================================
	 logic taken_ex, pred_taken;
	 logic [31:0] pc_correct, pc_pred_if, pc_pred;
	 assign pc_correct = (taken_ex)? alu_result_ex : pc_ex+4;
	 assign pc_pred_if = (pred_taken)? pc_pred:pc_4_if;
	 assign pc_next = (mispredict)? pc_correct : pc_pred_if;
    ...
    .update_EX(is_branch_ex | is_jal_ex | is_jalr_ex), //=is_branch
    .pc_EX(pc_ex),
    .target_EX(alu_result_ex), //=alu
    .taken_EX(taken_ex) 
	 );
    ...
    IF_ID REG_IF_ID(
        ...
        .flush_IF_ID(flush_if_id), // Xóa IF/ID
		  .pc_pred_IF(pc_pred), .pred_taken_IF(pred_taken),
		  .pc_pred_ID(pc_pred_id), .pred_taken_ID(pred_taken_id)
    );
    ...
    ID_EX REG_ID_EX( // Thanh ghi pipeline ID/EX
        ...
        .rs1_EX(rs1_ex), .rs2_EX(rs2_ex), .insn_vld_EX(insn_vld_ex), .o_ctrl_EX(o_ctrl_ex),
		  .pc_pred_EX(pc_pred_ex), .pred_taken_EX(pred_taken_ex)
    );
